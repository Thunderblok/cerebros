...
Run these in IEx as separate lines (don't paste the prose label):

```elixir
Cerebros.hello()
Cerebros.test_basic_functionality()
```

If you accidentally paste the descriptive text (e.g. "Initialize and verify…") you will get a SyntaxError like the one shown; only evaluate Elixir expressions (lines starting with a module, variable, # comment, or iex prompt).

2. Minimal NAS demo (as in README)
2 architectures × 1 trial × 2 epochs (fast)
```elixir
Cerebros.test_full_nas_run(
	input_shapes: [{10}],
	output_shapes: [{1}],
...
```elixir
Cerebros.test_full_nas_run(batch_size: 64, learning_rate: 0.005, epochs: 4)
Cerebros.test_full_nas_run(batch_size: 16, learning_rate: 0.001, epochs: 4)
```

6. Capturing the full result object
```elixir
{:ok, results} = Cerebros.test_full_nas_run(number_of_architectures_to_try: 2, number_of_trials_per_architecture: 1, epochs: 2)
results |> Enum.map(& &1.validation_loss)
```

7. Inspecting best trial manually
```elixir
best = Enum.min_by(results, fn r -> r.validation_loss || :infinity end)
best.trial_id
best.model_size
best.architecture
```
...
```elixir
{:ok, orch} = Cerebros.Training.Orchestrator.start_link(max_concurrent: 2)
search_cfg = %{
	input_shapes: [{10}],
	output_shapes: [{1}],
	number_of_architectures_to_try: 2,
...
GenServer.cast(orch, {:start_search, search_cfg})
```

Poll progress
```elixir
Cerebros.Training.Orchestrator.list_trials(orch)
:timer.sleep(3000)
Cerebros.Training.Orchestrator.get_results(orch)
GenServer.stop(orch)
```
...
conn_cfg = %{
	...
	maximum_skip_connection_depth: 3,
	...
	lateral_connection_decay: fn _ -> 0.85 end,
	max_consecutive_lateral_connections: 2,
	gate_after_n_lateral_connections: 3
}
spec = Cerebros.Architecture.Spec.random(conn_cfg,
	input_specs: [%{shape: {10}, dtype: :f32}],
	output_shapes: [1],
	min_levels: 2,
	max_levels: 3
)
{:ok, orch} = Cerebros.Training.Orchestrator.start_link(max_concurrent: 1)
{:ok, trial_id} = Cerebros.Training.Orchestrator.submit_trial(orch, spec, %{epochs: 3, batch_size: 32, learning_rate: 0.01, dataset: :cifar10})
Process.sleep(2000)
Cerebros.Training.Orchestrator.get_trial_status(orch, trial_id)
Cerebros.Training.Orchestrator.get_results(orch)
GenServer.stop(orch)
```
...
```elixir
input = Axon.input("x", shape: {nil, 10})
model = input |> Axon.dense(32, activation: :relu) |> Axon.dense(1)
{_graph, params, _state} = Axon.build(model, Nx.iota({1,10}, type: :f32))
param_total =
	params.data
	|> Map.values()
...
```elixir
Enum.each(1..5, fn _ ->
	IO.inspect(Cerebros.Training.Orchestrator.list_trials(orch))
	Process.sleep(1000)
end)
```
...
```elixir
{:ok, res} = Cerebros.test_full_nas_run(number_of_architectures_to_try: 3, number_of_trials_per_architecture: 1, epochs: 3)
Enum.map(res, &{&1.trial_id, &1.validation_loss})
Enum.sort_by(res, & &1.validation_loss)
```
...
```elixir
:timer.tc(fn ->
	Cerebros.test_full_nas_run(number_of_architectures_to_try: 1, number_of_trials_per_architecture: 1, epochs: 2)
end)
```
...
```elixir
sample = Nx.random_uniform({5,10})
preds = Axon.predict(model, params, sample)
Nx.shape(preds)
```

23. Logging best vs average improvement (post-run)
```elixir
{:ok, res2} = Cerebros.test_full_nas_run(number_of_architectures_to_try: 4, number_of_trials_per_architecture: 1, epochs: 2)
losses = Enum.map(res2, & &1.validation_loss) |> Enum.reject(&is_nil/1)
best = Enum.min(losses)
avg = Enum.sum(losses)/length(losses)
IO.puts("Improvement: #{Float.round((avg - best)/avg * 100, 2)}%")
```

24. Simple retry wrapper for flaky runs
```elixir
retry = fn fun, attempts ->
	Enum.reduce_while(1..attempts, nil, fn i, _ ->
		case fun.() do
			{:ok, v} -> {:halt, {:ok, v}}
			{:error, e} ->
				IO.puts("Attempt #{i} failed: #{inspect(e)}")
				if i == attempts, do: {:halt, {:error, e}}, else: {:cont, nil}
		end
	end)
end
retry.(fn ->
...
```elixir
{:ok, res3} = Cerebros.test_full_nas_run(number_of_architectures_to_try: 2, number_of_trials_per_architecture: 1, epochs: 2)
File.write!("nas_results.json", Jason.encode!(res3, pretty: true))
```
...
```elixir
inp = Axon.input("x", shape: {nil, 10})
mdl = inp |> Axon.dense(16, activation: :relu) |> Axon.dense(1)
loss_fun = :mean_squared_error
optimizer = Polaris.Optimizers.adam(0.001)
loop = Axon.Loop.trainer(mdl, &Axon.Losses.mean_squared_error/3, optimizer)
```
...
train_stream =
	Stream.repeatedly(fn ->
		x = Nx.random_uniform({32,10})
		y = Nx.sum(x, axes: [1], keep_axes: true)
		{x, y}
	end)
	|> Enum.take(20)

loop_params = Axon.Loop.run(loop, train_stream, %{}, epochs: 3)
```
...
```elixir
s1 = Cerebros.Architecture.Spec.random(conn_cfg, seed: 123, input_specs: [%{shape: {10}, dtype: :f32}], output_shapes: [1])
s2 = Cerebros.Architecture.Spec.random(conn_cfg, seed: 123, input_specs: [%{shape: {10}, dtype: :f32}], output_shapes: [1])
s1.levels == s2.levels
```
...
```elixir
{:ok, connectivity} = Cerebros.Connectivity.Builder.build_connectivity(spec)
map_size(connectivity)
```
...
Enum.each(1..3, fn run ->
	IO.puts("Run #{run}")
	{:ok, res} = Cerebros.test_full_nas_run(number_of_architectures_to_try: 2, number_of_trials_per_architecture: 1, epochs: 2)
	best = Enum.min_by(res, & &1.validation_loss)
	IO.inspect({run, best.validation_loss})
end)
```
...
35. Lorenz attractor demo (fixed step)
```elixir
lor = Cerebros.demo_lorenz(steps: 4000)
lor.spark_x
```

36. Lorenz attractor adaptive demo
```elixir
lor_ad = Cerebros.demo_lorenz(adaptive: true, tol: 1.0e-3, dt: 0.02)
lor_ad.spark_z
```

37. Export trajectory to CSV
```elixir
res = Cerebros.demo_pendulum(method: :rk4, dt: 0.01, t_end: 5.0)
Cerebros.Sim.ODE.export_csv(res, "tmp/pendulum.csv")
```

38. Sparkline of pendulum energy drift
```elixir
{spark, _, _} = Cerebros.Sim.ODE.ascii_sparkline(res.energy_over_time)
spark
```

39. Adaptive integration direct API
```elixir
f = Cerebros.Sim.ODE.lorenz_fun()
init = Nx.tensor([1.0,0.0,0.0])
ad = Cerebros.Sim.ODE.integrate_adaptive(init, 0.0, 5.0, f, tol: 1.0e-3)
Enum.take(Nx.to_flat_list(ad.dt_series), 5)
```
